...
class TestJoinInvariants:
    ...
    def test_zero_gap_passes(self, ahr, make_segment, utc):
        # Real ADS-B Exchange data sometimes shares the boundary instant between
        # adjacent placemarks for the same aircraft (split-point artifact).
        # The invariant should accept equality and let downstream join logic decide.
        a = make_segment("E47FA6", utc, duration_min=60,
                         start_lat=-23.4, start_lon=-46.5,
                         end_lat=-22.9, end_lon=-43.2)
